...
def main():
    ...
    if code_result.tests:
        print("🧪 TESTS:")
        print(code_result.tests)

    # Step 3: Execute safely in sandbox
    print("🛡️  SANDBOXED EXECUTION (Docker)...")
    try:
        result = coder.execute_code(code_result.code, timeout=30)
        print("✅ SUCCESS:")
    ...
    audit = exec_agent.self_audit(goal, code_result.code, result.stdout if 'result' in locals() else "")
    print("\n🔍 SELF-AUDIT REPORT:")
    print(audit)
